...
def game():
    ...
    for i in range(1,rows+1):
        for j in range(cols):
            x = (brickWidth + 5) * j
            y = (brickHeight + 5) * i
            bricks.append(pygame.Rect(x, y, brickWidth, brickHeight))
    # print(bricks)
    counter = 0
    lifeRemaining = 3
    FPS = 100
    ...
    while play:
        ...
        for i in range(len(bricks)):
            # check if brick get collide with ball rect
            if bricks[i].colliderect(ballRect):
                # ball will move back downward
                moveBallY = 4
                # score will increase by 1
                counter += 1
                # game speed will increase by 5 FPS
                FPS += 5
                # particular brick will get deleted from bricks list
                del bricks[i]
                # exit from loop
                break
        ...
        # check ball collision with boundaries or bar
        if ballX < 0:
            moveBallX = 4
